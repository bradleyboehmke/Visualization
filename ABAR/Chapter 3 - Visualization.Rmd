...
```{r qqplot, fig.height=3, fig.width=7}
par(mfrow = c(1, 2))
...
```{r bar1, fig.width=6, fig.height=4}
...
```{r bar2, fig.width=9, fig.height=3}
# total count
p1 <- ames %>% 
  count(MS_Zoning) %>%
...
```{r bar3, fig.width=6, fig.height=3}
ames %>% 
...
```{r dot, fig.height=6, fig.width=7}
ames %>%  
  count(Neighborhood) %>%
  mutate(pct = n / sum(n)) %>%
...
```{r dot1, fig.height=6, fig.width=7}
ames %>%  
  count(Neighborhood) %>%
  mutate(pct = n / sum(n)) %>%
  ggplot(aes(pct, reorder(Neighborhood, pct))) +
...
```{r ord1, fig.height=3, fig.width=5}
...
```{r ord2, fig.height=3, fig.width=5}
ames %>%
...
```{r int1, fig.width=7, fig.height=5}
...
```{r missing1, fig.height=4, fig.width=8}
MASS::survey %>%
...
```{r mislabeled, fig.height=3, fig.width=4}
...
Having a solid understanding of univariate distributions is important; however, most analyses want to take the next step to understand associations and relationships across different variables. Features we are generally interested in include:
...
```{r scatter1,  fig.height=3, fig.width=4}
...
```{r dbl_scatter, fig.width=8, fig.height=4}
...
```{r density_plot, fig.width=8, fig.height=3}
...
```{r int_scatter, fig.width=7, fig.height=5}
...
```{r joyplot, fig.width=7, fig.height=4}
...
```{r multidim1, fig.width=4, fig.height=3}
...
```{r smallmulti, fig.width=8, fig.height=5}
ggplot(ames, aes(x = Gr_Liv_Area, y = Sale_Price)) +
  geom_point(alpha = .3) +
  scale_x_log10() +
...
```{r multivar1, fig.width=8, fig.height=5}
ggplot(ames, aes(x = Gr_Liv_Area, y = Sale_Price, color = Central_Air, shape = Central_Air)) +
  geom_point(alpha = .3) +
...
```{r parallel1, fig.width=7, fig.height=4}
...
```{r parallel2, fig.width=7, fig.height=4}
ames %>%
  select(variables) %>%
...
One dimensional displays of categorical data were discussed in section 3.2.  However, similar to the last section, it is important to understand how a categorical response is associated with multiple variables. 

As in the last section we can use faceting (`facet_wrap` and `facet_grid`) to produce small multiples and also visualize factors with PCPs.  Mosaic plots provide an alternative approach for multivariate categorical data.  Mosaic plots are a graphical method for visualizing data from two or more qualitative variables. In this visual the graphics area is divided up into rectangles proportional in size to the counts of the combinations they represent.
...
mosaicplot(Above_Avg ~ Garage_Type + Garage_Cars + Garage_Qual, data = ames2, las = 1)
