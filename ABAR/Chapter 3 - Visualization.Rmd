...
```{r density, fig.align='center', fig.width=9, fig.height=3}
p1 <- ggplot(ames, aes(Sale_Price)) +
  geom_density()
...
```{r density_plot, fig.width=9, fig.height=3}
...
```{r facetw_bar, fig.height=3, fig.width=8}
ames %>%
  mutate(
...
```{r facetg_bar, fig.height=8, fig.width=8}
ames %>%
  mutate(
    Above_Avg = ifelse(Sale_Price > mean(Sale_Price), "Above", "Below"),
    Kitchen_Qual = fct_relevel(Kitchen_Qual, "Poor", "Fair", "Typical", "Good")
...
```{r multivar1, fig.width=10, fig.height=5}
ggplot(ames, aes(x = Gr_Liv_Area, y = Sale_Price, color = Central_Air, shape = Central_Air)) +
  geom_point(alpha = .3) +
...
```{r parallel2, fig.width=9, fig.height=4}
ames %>%
  select(variables) %>%
...
```{r mosaic, fig.height=6, fig.width=9}
ames2 <- ames %>%
  mutate(
...
```{r mosaic2, fig.height=5, fig.width=8}
ames %>% 
...
```{r missing2, fig.height=7, fig.width=12}
